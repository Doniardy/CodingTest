...
const App: React.FC = () => {
  ...
  const dictUpper = Object.entries(letterToNumber).filter(
    ([ch]) => ch === ch.toUpperCase() && ch !== " "
  );
  const dictLower = Object.entries(letterToNumber).filter(
  ...
  return (
    <div style={{ fontFamily: "monospace", padding: "20px" }}>
      ...
      <h2 style={{ marginTop: "40px" }}>Kamus Huruf Besar</h2>
      <table border={1} cellPadding={5} style={{ borderCollapse: "collapse" }}>
        <thead>
          <tr>{dictUpper.map(([ch]) => <th key={ch}>{ch}</th>)}</tr>
        </thead>
        <tbody>
          <tr>{dictUpper.map(([ch, num]) => <td key={ch}>{num}</td>)}</tr>
        </tbody>
      </table>

      <h2 style={{ marginTop: "40px" }}>Kamus Huruf Kecil</h2>
      <table border={1} cellPadding={5} style={{ borderCollapse: "collapse" }}>
        <thead>
          <tr>{dictLower.map(([ch]) => <th key={ch}>{ch}</th>)}</tr>
        </thead>
        <tbody>
          <tr>{dictLower.map(([ch, num]) => <td key={ch}>{num}</td>)}</tr>
        </tbody>
      </table>
    </div>
  );
